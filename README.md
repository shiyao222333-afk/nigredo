...
| 字幕缺失的视频干瞪眼 | 字幕三级兜底（CC → AI → Whisper） |
...
1. **字幕三级兜底**：CC 人工字幕 → B站 AI 字幕（WBI 直取，纯网络免 GPU）→ Whisper ASR，任何视频都出文字。
...
**Q3：字幕三级策略是什么？**
CC 人工字幕 → B站 AI 字幕（WBI 直取，免 GPU）→ Whisper ASR 兜底。三级接力确保任何视频都能出文字。
